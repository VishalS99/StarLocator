...
def _rotate(points: np.ndarray, theta: float) -> np.ndarray:
    return points @ np.array([[np.cos(theta), -
...
def synthesize_data(
    ...
    image = _clipped_normal(
        0.0, noise_level, 0, 1, size=(
            image_size, image_size))

    def noisy_draw(x, y):
        valid = (x >= 0) & (x < image_size) & (y >= 0) & (y < image_size)
        image[x[valid], y[valid]] = _clipped_normal(
            0.25, noise_level, 0, 1, size=np.sum(valid))

    # star (or no-star)
    if has_star:
        points, label = _star(_pos(image_size), _yaw(), _side_len(
        ), _width_stretch(), _height_stretch(), _roundness())
    ...
    for _ in range(n_lines):
        noisy_draw(*line(*np.random.randint(-image_size //
                                            2, image_size + image_size // 2, size=4)))
...
"""
Added normalization and denormalization of vectors for efficient training
"""
...
def normalize_data(label):
    ...
    alpha = alpha / (2 * np.pi)
    label = torch.stack([x, y, w, h, alpha], dim=-1)
    return label


def denormalize_data(label):
    x = label[:, 0]
    y = label[:, 1]
    w = label[:, 2]
    h = label[:, 3]
